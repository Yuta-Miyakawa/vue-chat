<template>
  ...
  <q-dialog v-model="isExportDialogOpen" persistent>
    <div class="q-gutter-y-md" style="width: 520px;">
      <q-card>
      <q-card-section>
        <!-- <div class="text-h6 text-center">インポート&エクスポート(Excel)</div> -->
        <div class="text-h6 text-center">エクスポート(Excel)</div>
        <!-- <div class="text-subtitle2 text-center">Excel</div> -->
      </q-card-section>

      <q-tabs v-model="tab" class="text-teal">
        <!-- <q-tab label="インポート" name="one" v-if="contacts=='ContactlistContact'"/> -->
      ...
      <q-tab-panels v-model="tab" animated>
        <q-tab-panel name="one">
          <q-card-section>
            <div class="text-h6 text-center">檀家をインポート
        ...
        <q-tab-panel name="two">
          <q-card-section>
            <div class="text-h6 text-center">檀家をエクスポート</div>
          </q-card-section>
          <p class="text-center">
            Excelにエクスポートできます。20ページ以内でエクスポートしてください。<br>
            1ページ(15件)ごとにエクスポートする檀家の数が決まります。
          </p>
...
export default defineComponent({
    ...
    setup(props, context){
      ...
      const ExportExcel = async (type, fn, dl) => {
        console.log('あいうえお',props.contacts)
        ...
        if(props.contacts === 'DanakaContact'){
           for (let i = start_page; i <= last_pages; i++) {
            ...
            const Data = $store.state.dankaModule.DankaData.data;
        ...
        return dl
          ...
          : XLSX.writeFile(wb, fn || tenantName + "-檀家名簿-" + dayjs(inputDeath.value).format('YYYYMMDD') + "." + (type || "xlsx"));
      };
