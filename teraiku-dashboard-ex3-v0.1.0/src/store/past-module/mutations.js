...
export default {
    createResource(state, payload) {
      ...
      // state.pastfamily.data.push(payload);
    },
    ReadResources(state, payload) {
      state.pastfamily = payload;
      console.log("state.pastfamily",state.pastfamily.data);
      for (let i = 0; i < state.pastfamily.data.length; i++) {
        // console.log(rows.value[i].date_of_death)
        const deathday = dayjs(state.pastfamily.data[i].date_of_death).format("YYYY-MM-DD");
    ...
    updateResource(state, payload) {
      state.pastfamily.data = state.pastfamily.data.map((past) =>
        payload.id === past.id ? { ...past, ...payload } : past
      );
    },
    deleteResource(state, payload) {
      console.log(state);
      state.pastfamily.data = state.pastfamily.data.filter((current) => current.id !== payload.id);
    },
    ReadResourcesProfile(state, payload) {
      console.log(state);
      state.pastfamily = pyaload;
    }
  };
