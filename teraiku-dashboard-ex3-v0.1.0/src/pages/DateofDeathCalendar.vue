<template>
  <q-page padding>
      ...
      <h5 style="margin: 10px">月命日 予定表</h5>
      <h7>
        檀家の月命日が表示されます。
      </h7>
      ...
      try {
        ...
        for (let i = 0; i < family.length; i++) {
          ...
          if(allFamily[0]){
            ...
            console.log("pastfamily in vue",pastfamily.value)
            for(let deathindex=0; deathindex<pastfamily.value.length; deathindex++){
              let deathday = pastfamily.value[deathindex].date_of_death;
              // console.log(deathday)
              for(let addmonth = 1; addmonth<12; addmonth++) {            
                if(deathday){
                  let event = {}
                  event['start'] = dayjs(currentDate).format("YYYY-") + dayjs(deathday).add(addmonth, "M").format("MM-DD");
                  event['end'] = dayjs(currentDate).format("YYYY-") + dayjs(deathday).add(addmonth, "M").format("MM-DD");
                  event['title'] =  pastfamily.value[deathindex].last_name + pastfamily.value[deathindex].first_name;
                  event['content'] = '命日 : ' + dayjs(deathday).format("YYYY年MM月DD日");
