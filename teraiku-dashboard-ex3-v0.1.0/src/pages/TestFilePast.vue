  ...
  <q-page class="q-pa-sm bg-white" >
    ...
    <div class="row" v-if="!$q.screen.lt.md"  >
          ...
          <q-tab-panels v-model="tab" animated class="bg-white">
            <q-tab-panel name="all" class="q-pa-none full-height" style="min-height: calc(100vh - 150px)">
              <q-list class="">
                ...
                <!-- <q-item-label header class="text-left">{{Tenant.corporation }} 過去帳一覧 {{ contentTotal }} 件</q-item-label> -->
                <q-item-label header class="text-left">
                  過去帳一覧:{{ contentTotal }} 件
                  <!-- <dialog-create :id="family" :contacts="selected_contact" :SelectCreateDialog="SelectCreateDialog='contactLists'" @updateROWS="showDankaCreate()"  @deleteDANKA="showDankaDelete()" @updateDANKA="showDankaUpdate(selected_contact)"></dialog-create> -->
                </q-item-label>
                <q-scroll-area
                ...
                >
                <span v-for="(contact, index) in rows" :key="index" @click="selected_contact=contact;SelectElement = true; DayFormat()">
                  <contact-item
                    :avatar="contact.avatar" :name="'俗名：' + contact.last_name + ' ' + contact.first_name" :label="dayofNext[index]" type="法要"></contact-item>
                </span>
      ...
      <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12" v-else>
        <q-card class="no-border no-border">
          <q-tab-panels v-model="tab" animated class="bg-white">
            <!-- <q-tab-panel name="all" class="q-pa-none full-height" style="min-height: calc(100vh - 80px)">
              <q-list class="">
                <h6 style="margin:5px">
                  <q-btn
                    size="sm"
                ...
                <q-separator/>
                <q-scroll-area
                  style="height: 1px; min-height: calc(100vh - 280px)"
                  :thumb-style="thumbStyle"
                  v-show="showSimulatedReturnData"
                >
                  <q-item-label header class="text-left">
                    過去帳:{{ pastbooksTotal }}人
                    <dialog-create :id="family" :SelectEditDialog="SelectEditDialog='pastbooksContact'" @updateROWS="fetchDogs()" @updateDANKA="showDankaUpdate(selected_contact)"></dialog-create>
                  </q-item-label>

                  <span v-for="(contact, index) in pastbooksContact" :key="index" @click="selected_contact=contact;SelectElement = true; DayFormat()">
                    <contact-item
                  ...
                  <q-item-label header class="text-left">
                    現在帳:{{ currentTotal }}人
                    <dialog-create :id="family" :SelectEditDialog="SelectEditDialog='currentContacts'" @updateROWS="fetchDogs()" @updateDANKA="showDankaUpdate(selected_contact)"></dialog-create>
                  </q-item-label>
      ...
      <div class="col-lg-8 q-pl-xs col-md-8 col-sm-12 col-xs-12" v-if="SelectElement">
        <div v-if="CreateElement">
          <contact-create-list :selected_contact="SelectEditDialog" :user_id="family[0].id" :tenant_id="TenantID" :profile="profileData" checkFlag="Create" @CreateContact="showMoreInformation(selected_contact)"></contact-create-list>
        </div>
        <div v-else-if="EditElement && family[0]">
          <contact-create-list :selected_contact="SelectEditDialog" :user_id="family[0].id" :tenant_id="TenantID" :profile_id="selected_contact.id" :profile="profileData" checkFlag="Edit" @CreateContact="fetchDogs()" @EditContact="fetchDogs()"></contact-create-list>
        </div>
        <div v-else-if="EditElement">
          <contact-create-list :selected_contact="SelectEditDialog" :tenant_id="TenantID" :profile_id="selected_contact.id" :profile="profileData" checkFlag="Edit" @CreateContact="fetchDogs()" @EditContact="fetchDogs()"></contact-create-list>
        </div>
        <div v-else>
          <h6 style="margin:5px">
          ...
          <q-card class="no-border no-border" style="min-height: calc(100vh - 75px)">
            ...
            <q-toolbar class="text-black ">
              ...
              <!-- <q-btn round flat icon="star_outline" color="yellow">
                ...
                <!-- <q-btn
                  v-if="selected_contact.type=='てらいく会員'&&DetailInformation"
                  class="create-button"
                  color="red"
                  label="檀家にする"
                ...
                <div v-if="family[0]">
                  <contact-delete :id="TenantID" :user_id="family[0].id" :contacts="selected_contact" @DeleteContact="fetchDogs()"></contact-delete>
                </div>
                <q-btn v-if="!selected_contact.corporation"  round size="sm" class="q-mx-xs" color="info" icon="edit" @click="SelectElement = true; CreateElement = false; EditElement = true; SelectEditDialog='pastbooksContact'; profileEdit(selected_contact)"/>
                <!-- <dialog-item :id="family" :contacts="selected_contact" :SelectEditDialog="SelectEditDialog='pastbooksContact'" @updateROWS="tenantDankaUpdate(selected_contact)" @deleteDANKA="showDankaDelete()" @updateDANKA="showDankaUpdate(selected_contact)"></dialog-item> -->
            ...
            <div v-if="PastbooksInformation">
              <div  v-for="detail, key, detail_index in detail_list" :key="key" >
                ...
                <contact-detail-item :icon="detail.icon" :text_color="detail.text_color" 
                                    ...
                                    :address="ConnectAddress" :birthday="birthdayInformation" :old="yearsOldInformation" :dayofNext="dayofNext[index]"></contact-detail-item>
            ...
            <div v-else>
              <div  v-for="detail, key, detail_index in pastbook_list" :key="key" >
                ...
                <contact-detail-item :icon="detail.icon" :text_color="detail.text_color"
                                    ...
                                    :deathday="deathdayInformation" :old="yearsOldInformation" :dayofNext="dayofNext[index]"></contact-detail-item>
    ...
    <div v-else>
            ...
            <q-tab-panel name="all" class="q-pa-none full-height" style="min-height: calc(100vh - 80px)">
              <q-list class="">
                ...
                <span v-for="(contact, index) in pastbooksContact" :key="index" @click="selected_contact=contact;SelectElement = true; SelectEditDialog='pastbooksContact'; DayFormat();">
                  <contact-item
                    :avatar="contact.avatar" :name="contact.last_name + ' ' + contact.first_name" :position="contact.type" :label="dayofNext[index]" type="法要"></contact-item>
                </span>
      ...
      >
        <div v-if="CreateElement">
          <contact-create-list :selected_contact="SelectEditDialog" :user_id="family[0].id" :tenant_id="TenantID" :profile="profileData" checkFlag="CreateContact" :back="true" @CreateContact="showMoreInformation(selected_contact)"></contact-create-list>
        </div>
        <div v-else-if="EditElement && family[0]">
          <contact-create-list :selected_contact="SelectEditDialog" :user_id="family[0].id" :tenant_id="TenantID" :profile_id="selected_contact.id" :profile="profileData" checkFlag="Edit" @CreateContact="fetchDogs()" @EditContact="fetchDogs()"></contact-create-list>
        </div>
          ...
          <q-card class="no-border no-border"
            ...
            <div v-if="PastbooksInformation">
              <div  v-for="detail, key, detail_index in detail_list" :key="key" >
                ...
                <contact-detail-item :icon="detail.icon" :text_color="detail.text_color"
                                    :value=" selected_contact[detail['field']]" :label="detail.label"
                                    :address="ConnectAddress" :birthday="birthdayInformation" :dayofNext="dayofNext[detail_index]"></contact-detail-item>
            ...
            <div v-else>
              <div  v-for="detail, key, detail_index in pastbook_list" :key="key" >
                ...
                <contact-detail-item :icon="detail.icon" :text_color="detail.text_color"
                                    :value=" selected_contact[detail['field']]" :label="detail.label"
                                    :address="ConnectAddress" :birthday="birthdayInformation"
                                    :deathday="deathdayInformation" :old="yearsOldInformation" :dayofNext="dayofNext[detail_index]"></contact-detail-item>
  ...
  setup(props, context) {
    ...
    const fetchDogs = async (page = 1) => {
      ...
      try {
        ...
        console.log("今ここです",$store.state.pastModule.pastfamily.data);
        rows.value = $store.state.pastModule.pastfamily.data;
        console.log("row見ます",rows.value)
        pastdeath.value = $store.state.pastModule.pastdeath;
    ...
    const DayFormat = () => {
      ...
      if(SelectElement.value === true){
        ...
        console.log(yearsOldInformation.value)
      } else {
        SelectElement.value = false;
      }
      CreateElement.value = false;
